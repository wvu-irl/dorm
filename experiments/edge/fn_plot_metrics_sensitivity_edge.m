function [] = fn_plot_metrics_sensitivity_edge(bsp, x, u, data)
% global robot_;
% global param_alpha_;
...
use_semilog=true;
if(use_semilog)
%     h3=semilogy([bsp.robot_.dt_*(0:1:size(x,2)-1)], [data(:).nuPOptimal],'linestyle','-','linewidth',linewidth,'DisplayName','$\overline{\nu}_t^+ | \overline{\nu}_{t-1}^+$'); hold on;
    h2=semilogy([bsp.robot_.dt_*(0:1:size(x,2)-1)], [data(:).phiPOptimal],'linestyle','-','linewidth',linewidth,'DisplayName','$\overline{\nu}_t^+ |\det[\Sigma_{t-1}^+]$'); hold on;
