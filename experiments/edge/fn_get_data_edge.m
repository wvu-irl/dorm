function [data] = fn_get_data_edge(bsp, x, u, P0)
...
%         P_test = reshape(b(robot_.x_dim_+1:end), [robot_.x_dim_,robot_.x_dim_]);
        ...
        num = lambdaFF*max_eigenvalue_t + lambdaQ;
        den = lambdaR*(lambdaFF*max_eigenvalue_t + lambdaQ) + 1;
        lambdaPNotProp = num/den;

        %nuP optimal alpha
        [nu_Qgrave,iota_R] = bsp.get_nuQgrave_iotaR(xt, ut, xtp1, max_eigenvalue_t);
        num = nuPOptimal + nu_Qgrave - max_eigenvalue_t^bsp.robot_.x_dim_;
        den = iota_R*(nuPOptimal + nu_Qgrave - max_eigenvalue_t^bsp.robot_.x_dim_) + 1;
        nuPOptimal = num/den;
        
        %phi(detP) using optimal alpha
        [nu_Qgrave,iota_R] = bsp.get_nuQgrave_iotaR(xt, ut, xtp1, max_eigenvalue_t);
        num = det(Pprev) + nu_Qgrave - max_eigenvalue_t^bsp.robot_.x_dim_;
        den = iota_R*(det(Pprev) + nu_Qgrave - max_eigenvalue_t^bsp.robot_.x_dim_) + 1;
        phiPOptimal = num/den;
...
%         P_test = reshape(b(robot_.x_dim_+1:end), [robot_.x_dim_,robot_.x_dim_]);
        ...
        %upper bound max eigenvalue
        lambdaPNotProp = lambdaFF*max_eigenvalue_t + lambdaQ;

        %propagate upper bound from t to t+1 (OPTIMAL ALPHA)
        nu_Qgrave = bsp.get_nuQgrave(x, u, max_eigenvalue_t);
        nuPOptimal = nuPOptimal + nu_Qgrave - max_eigenvalue_t^bsp.robot_.x_dim_;
        
        %bound at t+1 using value at t, \phi(\det(P_{t+1}))) optimal alpha
        nu_Qgrave = bsp.get_nuQgrave(x, u, max_eigenvalue_t);
        phiPOptimal = det(Pprev) + nu_Qgrave - max_eigenvalue_t^bsp.robot_.x_dim_;
    end
